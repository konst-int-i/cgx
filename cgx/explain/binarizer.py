...
class FeatureBinarizer(TransformerMixin):
    ...
    def transform(self, X):
        ...
        for c in data:
            ...
            if c in maps:
                ...
                colName = (str(c), '!=', str(maps[c].index[0])) if len(maps[c]) == 1 else (
                str(c), '==', str(maps[c].index[1]))
    ...
    def update(self, feat: str, thresh: float):
        """
        Updates the fitted binarizer by adding a custom feature-threshold that is used
        in the `transform` method. Note that this will lead to unequal bin sizes
        Args:
            feat (str): feature to add the threshold
            thresh (float): feature to be added

        Returns:
            self: udpated FeatureBinarizer
        """
        new_thresh = self.thresh[feat]
        new_thresh = np.append(new_thresh, thresh)
        new_thresh = np.sort(new_thresh)
        self.thresh[feat] = new_thresh
        return self

    def remove(self, feat: str, thresh: float):
        """
        Removes a rule specifying feature and value
        Args:
            feat:
            thresh:
        ...
        Returns:
        """
        arr = self.thresh[feat]
        assert thresh in arr, "The rule you are trying to remove is not present in the binarizer"
        arr = arr[arr != thresh]
        self.thresh[feat] = arr
        return self
